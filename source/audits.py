...
def cloudAdvisor(config, tenName):
    ...
    if cat:
        ...
        the.setWarn(tenName + ': Cloud Advisor Not Enabled !!')
    the.increamentGauge(the.gaugeIncNum*gaugeBreaks['simple'])
def caRecommendations(rc, tenName):
    state = rc.lifecycle_state
    name = re.sub('^cost-management-(.*)-name$', r"\1", rc.name)
    est = rc.estimated_cost_saving
    imp = getImportanceMapping(rc.importance)
    pending = 0; dismissed=0; postponed=0; implemented=0
    for c in rc.resource_counts:
        if c.status=="PENDING": pending=c.count
        elif c.status=="DISMISSED": dismissed=c.count
        elif c.status=="POSTPONED": postponed=c.count
        elif c.status=="IMPLEMENTED": implemented=c.count
    created = dateFormat(rc.time_created)
    updated = dateFormat(rc.time_updated)
    stateStart = dateFormat(rc.time_status_begin)
    stateEnd = giveAdashOnNothing(dateFormat(rc.time_status_end))
    the.cloudAdvisor[tenName].append([name,est,state,imp,pending,dismissed,postponed,implemented,created,updated,stateStart,stateEnd])
importance={
    'moderate':'Medium',
    'critical':'Critical',
    'high':'High',
}
def getImportanceMapping(i):
    i=i.lower()
    if i in importance: return importance[i]
    else: i.title()
def cloudGuard(config, tenName):
    incGaugeCG=the.gaugeIncNum*gaugeBreaks['simple']/2
    rootCompId=config['tenancy'] # or with getRootCompartmentID(tenName)
    cgCl = oci.cloud_guard.CloudGuardClient(config)
    ret=callCgService(tenName, rootCompId, 'Problems', cgCl.list_problems, cgProblems, incGaugeCG)
    if ret:
        the.setWarn(tenName + ': Cloud Guard Not Enabled !!')
        return
    callCgService(tenName, rootCompId, 'Recommendations', cgCl.list_recommendations, cgRecommendations, incGaugeCG)
def callCgService(tenName, rootCompId, service, ociFunc, locFunc, incGaugeCG):
    ...
    if res:
        ...
        return 'cloud-guard-not-enabled-on-this-tenancy'
    # on pass None will be returned
